...
def test_parser_facade_keeps_the_established_read_surface():
    assert parser.__all__ == [
        ...
        "extract_ini_namespace", "first_source", "line_source",
    ...
    assert parser.extract_ini_namespace is sections.extract_ini_namespace
    ...
    assert (parser.extract_controller_toggles
            is menu.extract_controller_toggles)
