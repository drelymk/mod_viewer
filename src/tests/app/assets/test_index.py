...
def test_geometry_hash_normalization(value, expected):
    assert normalize_geometry_hash(value) == expected
...
def test_gimi_accepts_known_category_and_weapon_layouts(tmp_path):
    ...
    player = root / "PlayerCharacterData" / "Asset01"
    ...
    assert [item["path"] for item in index["assets"]] == [
        "PlayerCharacterData/Asset01", "WeaponData/Bows/Bow"]
...
def test_gimi_merges_immediate_nested_hash_metadata_into_parent_asset(tmp_path):
    ...
    assert record["path"] == "PlayerCharacterData/Asset06"
    assert {(item["hash"], item["metadata"]) for item in record["geometry"]} == {
        ("a1a1a1a1", "PlayerCharacterData/Asset06/hash.json"),
        ("abcdef12",
         "PlayerCharacterData/Asset06/Asset06Face/hash.json"),
    }
...
def test_gimi_preserves_nested_metadata_for_shared_geometry_hash(tmp_path):
    ...
    assert geometry["metadataPaths"] == [
        "PlayerCharacterData/Asset01/hash.json",
        "PlayerCharacterData/Asset01/Face/hash.json",
    ]
...
def test_wwmi_index_reads_metadata_and_records_detail_path(tmp_path):
    ...
    assert geometry["detailMetadata"] == "Asset01/TextureUsage.json"
...
def test_wwmi_object_layout_is_supported(tmp_path):
    ...
    assert index["assets"][0]["path"] == "Asset01"
    assert index["assets"][0]["geometry"][0]["metadata"] == \
        "Asset01/a1a1a1a1/Metadata.json"


@pytest.mark.parametrize("asset_type, filename, payload", [
    ("GIMI", "hash.json", [{"ib": "a1a1a1a1", "object_indexes": [0]}]),
    ("WWMI", "Metadata.json", {
        "vb0_hash": "a1a1a1a1", "components": [{"index_offset": 0}]}),
])
def test_individual_asset01_folder_gets_specific_error(
...
def test_api_add_rebuild_and_delete_manage_index_transactionally(
    ...
    (root / "Asset01" / "hash.json").write_text("{", encoding="utf-8")
    ...
    (root / "Asset01" / "hash.json").write_text(
        json.dumps([{"ib": "a1a1a1a1", "object_indexes": [0]}]),
        encoding="utf-8")
