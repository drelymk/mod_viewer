/**
	...
	@author Lea Verou
	@modified 2026-08-14: uses a local DOM helper adapter for offline/CSP-safe operation;
	large documents debounce full-document highlighting to keep input responsive
*/
...
export default class PrismLive {
	...
	requestUpdate() {
		// Prism reparses and rebuilds the complete highlighted DOM. Doing that
		// synchronously for every keypress makes large mod INIs feel sticky.
		// While a deferred pass is pending, the host stylesheet exposes the
		// ordinary textarea text so newly typed characters remain visible.
		if (this.value.length < self.DEFERRED_HIGHLIGHT_LENGTH) {
		...
		this.wrapper.classList.add("prism-live-pending");
		clearTimeout(this.updateTimer);
	...
	update (force) {
		...
		if (!force && this.code.textContent === code && $(".token", this.code)) {
			// Already highlighted
			this.wrapper.classList.remove("prism-live-pending");
			return;
		}
...
Object.assign(self, {
	...
	DEFERRED_HIGHLIGHT_LENGTH: 8_000,
	HIGHLIGHT_DELAY: 200,
	CARET_INDICATOR: /(^|[^\\])\$(\d+)/g,
	snippets: {
		"test": "Snippets work!",
...
// The host page loads prism-live.css synchronously so editor metrics are ready
// before construction and no runtime resource loader is needed.
