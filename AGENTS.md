...
- After modifying code, use the incremental `/graphify . --update` flow to keep the graph current. Ensure `graphify-out/cache` is writable before starting the refresh; if the current execution context cannot write it, report the blocker instead of launching the refresh. Do not run the full-corpus `graphify update .` command unless the user explicitly requests a full rebuild.
